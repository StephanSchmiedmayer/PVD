...
def train(gpu, opt, output_dir, noises_init):
    ...
    optimizer = optim.Adam(
        model.parameters(), lr=opt.lr, weight_decay=opt.decay, betas=(opt.beta1, 0.999)
    )
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        for i, data in enumerate(dataloader):
            ...
            if opt.distribution_type == "multi" or (
                opt.distribution_type is None and gpu is not None
            ):
                x = x.cuda(gpu)
        ...
        if (epoch + 1) % opt.vizIter == 0 and should_diag:
            ...
            with torch.no_grad():
                x_gen_eval = model.gen_samples(
                    new_x_chain(x, 25).shape, x.device, clip_denoised=False
                )
                x_gen_list = model.gen_sample_traj(
                    new_x_chain(x, 1).shape, x.device, freq=40, clip_denoised=False
                )
            ...
            visualize_pointcloud_batch(
                "%s/epoch_%03d_x.png" % (outf_syn, epoch),
                x.transpose(1, 2),
                None,
                None,
                None,
        ...
        if (epoch + 1) % opt.saveIter == 0:
            ...
            if opt.distribution_type == "multi":
                ...
                model.load_state_dict(
                    torch.load(
                        "%s/epoch_%d.pth" % (output_dir, epoch),
...
def parse_args():
    ...
    parser.add_argument(
        "--lr", type=float, default=2e-4, help="learning rate for E, default=0.0002"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--grad_clip", type=float, default=None, help="weight decay for EBM"
    )
    parser.add_argument(
        "--lr_gamma", type=float, default=0.998, help="lr decay for EBM"
    )
